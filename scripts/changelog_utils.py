...
class ChangelogUtils:
    ...
    def get_project_root() -> str:
        ...
        try:
            ...
            try:
                ...
                if git_root.exists():
                    return str(git_root)
                raise ChangelogError("Git root directory does not exist")
    ...
    def _format_entry_title(
        ...
        if len(title_only) > max_line_length:
            return ChangelogUtils._format_long_title(
                escaped_title, commit_sha, repo_url, max_line_length
            )

        return ChangelogUtils._format_split_title(
            escaped_title, commit_sha, repo_url, max_line_length
        )

    @staticmethod
    def _format_long_title(
        escaped_title: str, commit_sha: str, repo_url: str, max_line_length: int
    ) -> list[str]:
    ...
    def _handle_existing_tag(tag_version: str, force_recreate: bool) -> None:
        ...
        try:
            ...
            if result_code == 0:
                ...
                print(
                    f"{COLOR_BLUE}Deleting existing tag '{tag_version}'...{COLOR_RESET}"
                )
    ...
    def _create_tag_with_message(tag_version: str, tag_message: str) -> None:
        ...
        print(
            f"{COLOR_BLUE}Creating tag '{tag_version}' with changelog content...{COLOR_RESET}"
        )
    ...
    def _show_success_message(tag_version: str) -> None:
        ...
        print("Next steps:")
        print(
            f"  1. Push the tag: {COLOR_BLUE}git push origin {tag_version}{COLOR_RESET}"
        )
        print(
            f"  2. Create GitHub release: {COLOR_BLUE}gh release create {tag_version} --notes-from-tag{COLOR_RESET}"
        )
...
def _enhance_with_ai(file_paths: dict[str, Path], project_root: Path) -> None:
    ...
    # Try to find enhance_commits.py in multiple locations
    script_locations = [
        Path(__file__).parent / "enhance_commits.py",  # Same directory as this script
...
def _cleanup_final_output(file_paths: dict[str, Path]) -> None:
    """Clean up final output to be markdownlint compliant."""
